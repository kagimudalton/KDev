...
fn linux_environment() -> Result<LinuxEnvironment, String> {
    ...
    {
        let wsl = Command::new("wsl.exe").args(["--status"]).output();
        if let Ok(output) = wsl {
            if output.status.success() {
                let text = String::from_utf8_lossy(&output.stdout).trim().to_string();
                let detail = if text.is_empty() {
                    "WSL is available. Install a Kali Linux distribution to use the full Kali environment.".to_string()
                } else {
                    text
                };
                return Ok(LinuxEnvironment { available: true, provider: "WSL2".into(), detail });
            }
        }
        return Ok(LinuxEnvironment {
            available: false,
            provider: "WSL2".into(),
            detail: "WSL2 is not available on this host. The Linux workspace UI is still available offline.".into(),
        });
    }
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        Ok(LinuxEnvironment {
            available,
            provider: "native Linux shell".into(),
...
fn run_linux_command(command: String) -> Result<String, String> {
    ...
    #[cfg(target_os = "windows")]
    let output = {
        let kali_probe = Command::new("wsl.exe")
            .args(["-l", "-q"])
            .output();
        let has_kali = kali_probe
            .ok()
            .map(|o| String::from_utf8_lossy(&o.stdout).to_lowercase().contains("kali"))
            .unwrap_or(false);

        if has_kali {
            Command::new("wsl.exe")
                .args(["-d", "kali-linux", "--", "bash", "-lc", command])
                .output()
                .map_err(|e| format!("failed to start Kali through WSL: {e}"))?
        } else {
            Command::new("wsl.exe")
                .args(["--", "bash", "-lc", command])
                .output()
                .map_err(|e| format!("failed to start Linux shell through WSL: {e}"))?
        }
    };
