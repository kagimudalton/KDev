...
#[cfg(windows)]
fn windows_path_to_wsl(path: &Path) -> String { let raw = path.to_string_lossy().replace('\\', "/"); let bytes = raw.as_bytes(); if bytes.len() >= 2 && bytes[1] == b':' { format!("/mnt/{}/{}", (bytes[0] as char).to_ascii_lowercase(), &raw[2..].trim_start_matches('/')) } else { raw } }
...
fn run_linux_command(command: String, working_directory: Option<String>) -> Result<String, String> {
    ...
    #[cfg(target_os = "windows")]
    let output = { let distro = preferred_wsl_distro().ok_or_else(|| "No WSL Linux distribution is installed.".to_string())?; let cwd = match working_directory { Some(p) => windows_path_to_wsl(&safe_relative_target(&p)?), None => windows_path_to_wsl(&kdev_root()?) }; let shell_command = format!("cd '{}' && {}", cwd.replace('\'', "'\\''"), command); Command::new("wsl.exe").args(["-d", &distro, "--", "bash", "-lc", &shell_command]).output().map_err(|e| format!("failed to start Linux shell: {e}"))? };
    #[cfg(not(target_os = "windows"))]
    let output = { let cwd = match working_directory { Some(p) => safe_relative_target(&p)?, None => kdev_root()? }; Command::new("bash").args(["-lc", command]).current_dir(cwd).output().map_err(|e| format!("failed to start bash: {e}"))? };
...
fn run_dev_command(command: String, working_directory: Option<String>) -> Result<String, String> {
    let command = command.trim(); if command.is_empty() { return Ok(String::new()); } if command.len() > 4096 { return Err("command is too long".into()); } let cwd = match working_directory { Some(p) => safe_relative_target(&p)?, None => kdev_root()? };
    #[cfg(windows)] let output = Command::new("powershell.exe").args(["-NoProfile", "-NonInteractive", "-Command", command]).current_dir(cwd).output().map_err(|e| format!("failed to start PowerShell: {e}"))?;
    #[cfg(not(windows))] let output = Command::new("bash").args(["-lc", command]).current_dir(cwd).output().map_err(|e| format!("failed to start shell: {e}"))?;
    let mut text = String::from_utf8_lossy(&output.stdout).to_string(); let stderr = String::from_utf8_lossy(&output.stderr); if !stderr.is_empty() { text.push_str(&stderr); } if !output.status.success() && text.trim().is_empty() { text = format!("command exited with status {}", output.status); } Ok(text)
}

#[tauri::command]
fn create_file(relative_path: String, content: String) -> Result<(), String> { let path = safe_workspace_path(&relative_path)?; if path.exists() { return Err("file or folder already exists".into()); } if let Some(parent) = path.parent() { fs::create_dir_all(parent).map_err(|e| format!("cannot create parent directory: {e}"))?; } let mut file = fs::File::create(&path).map_err(|e| format!("cannot create file: {e}"))?; file.write_all(content.as_bytes()).map_err(|e| format!("cannot write file: {e}"))?; file.sync_all().map_err(|e| format!("cannot flush file: {e}"))?; Ok(()) }
#[tauri::command]
