...
struct PlatformInfo {
    ...
    arch: String,
    kdev_root: String,
    /// True when KDev is running in true "flash drive" portable mode (its
    /// data folder lives next to the executable). False means it has been
    /// installed and is using a per-user writable fallback location.
    portable: bool,
}
...
fn safe_project_name(name: &str) -> Result<String, String> {
    ...
    if name.is_empty() || name.len() > 80 {
        return Err("project name must be 1-80 characters".into());
    }
...
fn safe_relative_target(relative_path: &str) -> Result<PathBuf, String> {
    safe_workspace_path(relative_path)
}

/// Strips anything that could break a Windows `Command` argument -- NUL
/// bytes most of all, since Rust's `Command` refuses arguments containing
/// them -- or that could otherwise smuggle unexpected characters into a
/// shell invocation. WSL distro names should only ever contain ordinary
/// identifier-like characters, so this is safe to apply unconditionally.
fn sanitize_command_token(raw: &str) -> String {
    raw.chars().filter(|c| !c.is_control()).collect::<String>().trim().to_string()
}

/// `wsl.exe -l -q` prints its distro list as UTF-16LE on Windows (often
/// with a UTF-16LE byte-order-mark). Decoding that as UTF-8 misinterprets
/// the two-bytes-per-character encoding, which produces stray NUL bytes and
/// mangled characters; passing those straight into `Command` later causes
/// "failed to start Linux shell: nul byte found in provided data". This
/// explicitly detects and strips a BOM, falls back to a byte-pattern
/// heuristic when there is none, and -- regardless of which decoding path
/// was used -- defensively strips any remaining control characters
/// (including embedded NULs) from every resulting line before it is ever
/// used again.
fn decode_wsl_output(raw: &[u8]) -> String {
    ...
    if !is_utf16 && even_len >= 2 && bytes[..even_len].chunks_exact(2).all(|c| c[1] == 0) {
        is_utf16 = true;
    }
    if is_utf16 {
        let units: Vec<u16> = bytes[..even_len].chunks_exact(2).map(|c| u16::from_le_bytes([c[0], c[1]])).collect();
...
fn wsl_distros() -> Vec<String> {
    ...
    text.lines().map(sanitize_command_token).filter(|s| !s.is_empty()).collect()
}
#[cfg(windows)]
...
fn platform_info() -> Result<PlatformInfo, String> {
    ...
    let portable = std::env::current_exe()
        ...
        .map(|exe_dir| root.starts_with(&exe_dir))
        .unwrap_or(false);
    Ok(PlatformInfo {
        os: std::env::consts::OS.into(),
        arch: std::env::consts::ARCH.into(),
        kdev_root: root.display().to_string(),
        portable,
    })
}
...
fn linux_environment() -> Result<LinuxEnvironment, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(kali) = distros.iter().find(|d| d.to_lowercase().contains("kali")) {
            return Ok(LinuxEnvironment {
                available: true,
                provider: format!("WSL2 / {kali}"),
                detail: format!("Kali Linux distribution detected: {kali}"),
            });
        }
        if !distros.is_empty() {
            return Ok(LinuxEnvironment {
                available: true,
                provider: "WSL2".into(),
                detail: format!("Linux distribution detected: {}. Install Kali separately if you want the Kali environment.", distros.join(", ")),
            });
        }
        ...
        if status.map(|o| o.status.success()).unwrap_or(false) {
            return Ok(LinuxEnvironment {
                available: false,
                provider: "WSL2".into(),
                detail: "WSL is installed, but no Linux distribution is registered.".into(),
            });
        }
        return Ok(LinuxEnvironment {
            available: false,
            provider: "WSL2".into(),
            detail: "WSL2 is not available on this host. The Linux workspace UI remains available.".into(),
        });
    }
    #[cfg(not(target_os = "windows"))]
    {
        ...
        Ok(LinuxEnvironment {
            available,
            provider: "native shell".into(),
            detail: if available { format!("Shell detected: {shell}") } else { "No compatible shell was detected.".into() },
        })
    }
}
...
fn windows_path_to_wsl(path: &Path) -> String {
    ...
    if bytes.len() >= 2 && bytes[1] == b':' {
        format!("/mnt/{}/{}", (bytes[0] as char).to_ascii_lowercase(), &raw[2..].trim_start_matches('/'))
    } else {
        raw
    }
}
...
fn run_linux_command(command: String, working_directory: Option<String>) -> Result<String, String> {
    ...
    let output = {
        ...
        let cwd = match working_directory {
            Some(p) => windows_path_to_wsl(&safe_relative_target(&p)?),
            None => windows_path_to_wsl(&kdev_root()?),
        };
        let shell_command = format!("cd '{}' && {}", cwd.replace('\'', "'\\''"), command);
        Command::new("wsl.exe")
            .args(["-d", &distro, "--", "bash", "-lc", &shell_command])
            .output()
            .map_err(|e| format!("failed to start Linux shell: {e}"))?
    };
    #[cfg(not(target_os = "windows"))]
    let output = {
        let cwd = match working_directory {
            Some(p) => safe_relative_target(&p)?,
            None => kdev_root()?,
        };
...
fn create_file(relative_path: String, content: String) -> Result<(), String> {
    ...
    if path.exists() {
        return Err("file or folder already exists".into());
    }
...
fn delete_workspace_entry(relative_path: String) -> Result<(), String> {
    ...
    if normalized == "workspace" || normalized == "workspace/projects" || normalized.ends_with("/projects") {
        return Err("protected workspace directory".into());
    }
    if !path.exists() {
        return Err("entry does not exist".into());
    }
    if path.is_dir() {
        fs::remove_dir_all(path).map_err(|e| format!("cannot delete folder: {e}"))
    } else {
...
fn rename_workspace_entry(relative_path: String, new_name: String) -> Result<String, String> {
    ...
    if name.is_empty() || name == "." || name == ".." || name.chars().any(|c| c == '/' || c == '\\' || c.is_control()) {
        return Err("invalid new name".into());
    }
    let target = path.parent().ok_or_else(|| "cannot determine parent directory".to_string())?.join(name);
    if target.exists() {
        return Err("target already exists".into());
    }
    fs::rename(&path, &target).map_err(|e| format!("cannot rename entry: {e}"))?;
    let root = kdev_root()?;
    Ok(target.strip_prefix(root).unwrap_or(&target).to_string_lossy().replace('\\', "/"))
}
#[tauri::command]
fn create_project(name: String, template: String) -> Result<Vec<String>, String> {
    security::require_unlocked()?;
    let name = safe_project_name(&name)?;
    let root = kdev_root()?.join("workspace/projects").join(&name);
    if root.exists() {
        return Err(format!("project '{name}' already exists"));
    }
    fs::create_dir_all(&root).map_err(|e| format!("cannot create project: {e}"))?;
    let files: Vec<(&str, &str)> = match template.as_str() {
        ...
        "react" => vec![
            ...
            ("README.md", "# KDev React Project\n\nDependency versions in package.json are pinned for a reproducible install, not left as \"latest\".\n\n## Offline note\n\nKDev creates these project files entirely offline. Installing the dependencies listed in package.json (via \"Install Dependencies\" or `npm install`) still requires internet access the first time, since KDev does not bundle an offline npm package mirror. Until dependencies are installed, this project will not run.\n"),
        ],
        "next" => vec![
        ...
        "typescript" => vec![
            ("main.ts", "const message: string = 'Hello from KDev TypeScript';\nconsole.log(message);\n"),
    ...
    for (relative, content) in files {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).map_err(|e| format!("cannot create project directory: {e}"))?;
        }
        let mut file = fs::File::create(&path).map_err(|e| format!("cannot create {relative}: {e}"))?;
        file.write_all(content.as_bytes()).map_err(|e| format!("cannot write {relative}: {e}"))?;
        file.sync_all().map_err(|e| format!("cannot flush {relative}: {e}"))?;
        created.push(format!("workspace/projects/{name}/{relative}"));
    }
    Ok(created)
}
#[tauri::command]
fn write_workspace_file(relative_path: String, content: String) -> Result<(), String> {
    ...
    #[cfg(windows)]
    if path.exists() {
        fs::remove_file(&path).map_err(|e| format!("cannot replace existing file: {e}"))?;
    }
...
fn list_workspace_files() -> Result<Vec<String>, String> {
    ...
    for entry in WalkDir::new(&root).into_iter().filter_map(Result::ok) {
        if entry.file_type().is_file() {
            if let Ok(path) = entry.path().strip_prefix(&kdev) {
...
fn list_projects() -> Result<Vec<String>, String> {
    security::require_unlocked()?;
    let root = kdev_root()?.join("workspace/projects");
    if !root.exists() {
        return Ok(Vec::new());
    }
    let mut projects = Vec::new();
    for entry in fs::read_dir(root).map_err(|e| format!("cannot read projects: {e}"))?.filter_map(Result::ok) {
...
fn storage_status() -> Result<String, String> {
    security::require_unlocked()?;
    storage::storage_snapshot(&kdev_root()?)
}

#[tauri::command]
fn doctor_report(app: tauri::AppHandle) -> Result<Vec<runtime::ToolStatus>, String> {
    Ok(runtime::doctor_report(&app, &kdev_root()?))
}
...
pub fn run() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![
            ...
            security::lock_workspace,
            project_tools::project_info,
            project_tools::install_project_dependencies,
            project_tools::start_web_preview,
            project_tools::run_project_file,
            project_tools::format_project_file,
            debugger::debugger_capability,
            debugger::debug_python_command,
            extensions::list_extensions,
            extensions::set_extension_enabled,
            recovery::save_recovery_snapshot,
            recovery::list_recovery_snapshots,
            recovery::read_recovery_snapshot,
        ])
