...
fn run_linux_command(command: String) -> Result<String, String> {
    ...
    #[cfg(target_os = "windows")]
    let output = { let distro = preferred_wsl_distro().ok_or_else(|| "No WSL Linux distribution is installed.".to_string())?; Command::new("wsl.exe").args(["-d", &distro, "--", "bash", "-lc", command]).output().map_err(|e| format!("failed to start Linux shell: {e}"))? };
    #[cfg(not(target_os = "windows"))]
...
pub fn run() { tauri::Builder::default().invoke_handler(tauri::generate_handler![platform_info, linux_environment, run_linux_command, create_project, write_workspace_file, read_workspace_file, list_workspace_files, list_projects]).run(tauri::generate_context!()).expect("error while running KDev"); }
