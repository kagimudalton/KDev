...
type Runtime = { name: string; version: string; ok: boolean };
type Storage = { used: number; free: number; total: number; percent: number };
type Recovery = { path: string; size: number; modified: string };
...
export default function FinishCenter({ onClose }: Props) {
  ...
  async function refresh() {
    ...
    const names = ["Python", "Node.js", "npm", "Git"];
    const found: Runtime[] = [];
    for (let i = 0; i < commands.length; i++) {
      try { const text = await invoke<string>("run_dev_command", { command: commands[i], workingDirectory: undefined }); found.push({ name: names[i], version: text.trim().split("\\n")[0] || "available", ok: true }); }
      catch { found.push({ name: names[i], version: "not detected", ok: false }); }
    }
    setRuntimes(found);
    try {
      const raw = await invoke<string>("storage_snapshot", {});
      ...
      setStorage({ used, free, total, percent: values.filesystem_usage_percent ?? 0 });
    } catch {}
  }
  ...
  async function scanRecovery() {
    ...
    try {
      const command = navigator.platform.toLowerCase().includes("win")
        ? "if(Test-Path '.kdev/recovery'){ Get-ChildItem '.kdev/recovery' -Recurse -File | ForEach-Object { \"$($_.FullName)`t$($_.Length)`t$($_.LastWriteTime.ToString('o'))\" } }"
        : "if [ -d .kdev/recovery ]; then find .kdev/recovery -type f -printf '%p\\t%s\\t%TY-%Tm-%TdT%TH:%TM:%TS\\n'; fi";
      const text = await invoke<string>("run_dev_command", { command, workingDirectory: undefined });
      const rows = text.trim() ? text.trim().split("\\n").map(line => { const [path, size, modified] = line.split("\\t"); return { path, size: Number(size) || 0, modified: modified || "" }; }) : [];
      setRecovery(rows);
  ...
  return <div className="finish-overlay"><section className="finish-window">
    ...
    <div className="finish-layout"><aside>{tabs.map(([id, label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{label}</button>)}</aside>
      <main>
        ...
        {tab === "runtime" && <section><h3>Portable Runtime Manager</h3><p className="muted">KDev prefers runtimes shipped beside the application. Host detection is used as a development fallback.</p><div className="runtime-list">{runtimes.map(x => <div key={x.name}><span className={x.ok ? "ok-dot" : "bad-dot"}></span><b>{x.name}</b><code>{x.version}</code></div>)}</div><button className="primary" disabled={busy} onClick={() => void refresh()}>Refresh runtimes</button></section>}
        {tab === "recovery" && <section><h3>Recovery Center</h3><p className="muted">Snapshots are stored locally. Network access is never required for recovery.</p><button className="primary" disabled={busy} onClick={() => void scanRecovery()}>Scan recovery</button><div className="snapshot-list">{recovery.map(x => <div key={x.path}><b>{x.path}</b><span>{fmt(x.size)} · {x.modified}</span></div>)}</div></section>}
