...
export default function SystemCenter(){
 ...
 async function refresh(){
   ...
   try{const win=navigator.platform.toLowerCase().includes("win");const cmd=win?`$d=[IO.DriveInfo]::new((Get-Location).Path.Substring(0,2)); \"$($d.TotalSize),$($d.AvailableFreeSpace)\"`:`df -Pk . | tail -1 | awk '{print $2,$4}'`;const raw=(await invoke<string>("run_dev_command",{command:cmd,workingDirectory:undefined})).trim();const parts=raw.split(/[ ,]+/).map(Number).filter(Number.isFinite);if(parts.length>=2){const total=win?parts[0]:parts[0]*1024,free=win?parts[1]:parts[1]*1024;setStorage({used:total-free,free,total,percent:total?((total-free)/total)*100:0});}}catch{}
 }
