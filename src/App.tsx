...
function languageFor(path: string) {
  ...
  return ({ py: "python", js: "javascript", jsx: "javascript", ts: "typescript", tsx: "typescript", html: "html", css: "css", json: "json", md: "markdown" } as Record<string, string>)[ext ?? ""] ?? "plaintext";
}
    ...
    <div className="app-shell">
      ...
      <main className="workbench">
        <aside className="explorer panel">
          ...
          <div className="tree-root">KDev</div>
          <div className="tree-folder">workspace</div>
          <div className="tree-folder indent">projects</div>
          <div className="tree-folder indent-2">Welcome</div>
          {files.map((file) => (
            <button key={file.path} className={`tree-file ${file.path === activePath ? "active" : ""}`} onClick={() => setActivePath(file.path)}>
              <span className="file-icon">{file.name.endsWith(".py") ? "◆" : file.name.endsWith(".css") ? "#" : file.name.endsWith(".html") ? "<>" : "JS"}</span>
              {file.name}
            </button>
          ))}
        </aside>
        ...
        <section className="editor-area">
          <div className="tabs">
            <div className="tab active"><span>{activeFile.name}</span><span className="tab-dot">{saved ? "" : "●"}</span><button onClick={saveCurrentFile}>×</button></div>
          </div>
          ...
          <div className="terminal">
            <div className="terminal-tabs"><span className="selected">TERMINAL</span><span>OUTPUT</span><span>PROBLEMS</span><button onClick={() => setTerminal("")}>Clear</button></div>
            <pre>{terminal}</pre>
          </div>
        </section>
        ...
        <aside className="side-panel panel">
          ...
          <div className="outline-item">{activeFile.name}</div>
          <div className="outline-item muted">Symbols will appear here as language intelligence is connected.</div>
          <div className="panel-title lower"><span>PROJECT</span></div>
          <div className="project-card"><strong>Welcome</strong><span>Portable workspace</span><span>Autosave: {saved ? "saved" : "saving…"}</span></div>
          <div className="panel-title lower"><span>KDEV</span></div>
          <div className="about">Built & developed by <strong>KAGIMU DALTON</strong><br /><span>Portable • Offline-first • Developer focused</span></div>
        </aside>
      </main>
      ...
      <footer className="statusbar">
        ...
        <span>{platform ? `${platform.os} • ${platform.arch}` : "Detecting platform…"}</span>
        <span className="status-spacer" />
        <span>UTF-8</span><span>Spaces: 2</span><span>Ln 1, Col 1</span>
      </footer>
    </div>
  );
}
